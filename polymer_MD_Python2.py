...
####################################################################
    ...
    def sizeFoldedProtein():
        size = 1/(2*(N^2))
        sum = 0
        for i in range(0, N):
            for j in range(i + 1, N):
                sum = sum + ((x[i][0] - x[j][0])**2 + (x[i][1] - x[j][0])**2)
        size = size * sum
        return size
    def plotGyration(distances, mytitle):
        # X = [i[0] for i in x]
        # Y = [i[1] for i in x]
        # colors = (0,0,0)
...
#####################################################################
    ...
    for step_i in range(0, steps): #molecular dynamics loop
        ...
        # print (sizeFoldedProtein())
        distances.append(sizeFoldedProtein())
    plotGyration(distances, "Gyration Distance")
...
#     for i in range(0, N):
#         for j in range(i + 1, N):
#             sum = sum + (X[i][0] - X[j][0]) + (X[i][1] - X[j][0])
#     size = size * sum
            # X[i][0] x[i][1]
            # X[j][0] x[j][1]
            # x[0]
